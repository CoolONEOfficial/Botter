...
public extension Bot {
    ...
    func getUser(from userFetchable: UserFetchable, app: Application) throws -> Future<User>? {
        ...
        case let .id(userId):
            ...
            return try vk?.getUser(params: .init(userIds: [.id(userId)], fields: nil, nameCase: nil)).map { User(from: $0.first) }.unwrap(orError: Bot.GetUserError.emptyArray)
...
extension Message {
    var vkUserParams: Vkontakter.Bot.GetUserParams? {
        ...
        return .init(userIds: [.id(userId)], fields: nil, nameCase: nil)
    }
}
